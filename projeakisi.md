...
- **Girdi Doğrulama (Input Validation):** Sisteme yüklenen dosyalar ve API istekleri kötü amaçlı yazılımlara ve manipülasyonlara karşı sıkı bir taramadan geçirilecektir.
 ...
 Problemin Tanımlanması, Algoritma Seçimi ve Sistem Mimarisi Tasarımı

**Yapılan Çalışmalar ve Analizler:**
* **Proje Konusunun Netleştirilmesi:** Akıllı Teşhis ve Tedavi Sistemi için tıbbi görüntülerin analizi (X-Ray, MRI vb.) üzerinde çalışılması kararlaştırıldı. Sistemin temel amacı; hastalıkları erken evrede teşhis etmek ve hekimlere kişiselleştirilmiş tedavi önerileri sunmaktır.
* **Algoritma ve Model Araştırması (Enes Zukra):** Görüntü işleme ve sınıflandırma görevlerinde yüksek başarı gösteren Evrişimli Sinir Ağları (CNN - Convolutional Neural Networks) mimarisi üzerinde detaylı bir literatür taraması yapıldı. Derin öğrenme tabanlı bu mimarinin, projenin temel taşını oluşturmasına karar verildi.
* **Geliştirme Ortamının Hazırlanması:** Projede kullanılacak kütüphaneler (TensorFlow, Keras, OpenCV, NumPy) belirlendi ve uyumluluk testleri yapıldı.
* **Veri Seti Arayışı:** Modeli eğitmek için kullanılacak açık kaynaklı tıbbi veri setleri (Kaggle vb.) incelendi ve veri ön işleme gereksinimleri listelendi.

**Sonuç ve Durum:**
Sistemin teorik altyapısı kuruldu, CNN mimarisinin kullanılacağı kesinleşti ve projenin yol haritası (Hafta 4, 5 ve 6'ya uzanan süreç) bu aşamada planlandı.
...
- Ekip görev dağılımı güncellenerek kod içi notlar doğru kişilere yönlendirilmiştir:  
  ...
  - Görüntü ön işleme entegrasyonu → Selim Yağbasan (5. ve 6. Hafta görevi)  
  - CNN model entegrasyonu → Enes Zukra (5. ve 6. Hafta görevi)  
...
### **API ve MySQL Veritabanı Entegrasyonu**

**Sorumlu:** Ali İstanbullu  
**Tarih:** 26 Nisan 2026  

Projenin 5. haftasında, daha önce tasarlanan Flask API altyapısının MySQL veritabanı ile tam entegrasyonu sağlanarak, sistemin sahte (dummy) verilerden kurtulup gerçek verilerle çalışabilmesi için gereken arka plan kodlamaları tamamlanmıştır.

#### **Yapılan Çalışmalar ve Entegrasyon Adımları**

- **MySQL Bağlantısının Kurulması:** `app.py` içerisine `mysql-connector-python` kütüphanesi entegre edilmiş ve Edanur Yasak tarafından tasarlanan ilişkisel `mediai_db` veritabanı ile API arasında iletişim köprüsü kurulmuştur.
- **Gerçek Veri Akışı (POST /api/v1/hastalar):** Web arayüzünden form aracılığıyla gönderilen hasta bilgileri (yaş, cinsiyet, hastalık geçmişi, diyabet durumu) API tarafından JSON formatında karşılanmış ve `Patients` tablosuna başarıyla kaydedilmesi sağlanmıştır.
- **Siber Güvenlik Önlemleri (SQL Injection Koruması):** Projenin 2. haftasındaki güvenlik analizinde belirtilen zafiyetleri önlemek amacıyla, veritabanına veri yazma işlemleri doğrudan değil; **Parametrik Sorgular (`%s`)** kullanılarak tamamen güvenli hale getirilmiştir.
- **Versiyon Kontrolü ve Birleştirme:** Yazılan veritabanı entegrasyon kodları, Cansude Sayın tarafından 5. haftada `app.py` üzerine eklenen CORS desteği, HTTP hata yönetimi ve veri doğrulama (validation) blokları ile hiçbir çakışma yaşanmadan birleştirilmiş (Merge) ve sistemin stabilitesi korunmuştur.
## 6. Hafta  (UYARI : 5 . hafta görevlerinizi yukarıya yazınız bu final senaryosudur )

### **Final Sunumu Hazırlığı ve Demo Senaryolarının Oluşturulması**
**Sorumlu:** Ali İstanbullu  
**Tarih:** 26 Nisan 2026  

Projenin final aşaması için kapsamlı bir sunum içeriği ve sistemin çalışma prensiplerini somutlaştıran demo senaryoları hazırlanmıştır.

- **Sunum İçeriği:** Projenin amacı, kullanılan teknolojiler (Flask, MySQL, CNN, OpenCV) ve sistemin klinik/operasyonel etkileri slayt bazlı planlanmıştır.
- **Demo Senaryosu:** 48 yaşında bir hastanın (Ahmet Yıldız) teşhis ve tedavi sürecinin; veri girişinden API tetiklenmesine, AI analizinden veritabanı kaydına kadar olan tüm adımları "Doktor Görüşü" perspektifiyle kağıda dökülmüştür.
- Tüm endpoint'lerin nasıl çalıştığını, request/response örneklerini, hata kodları tablosunu ve entegrasyon kontrol listesini içeren api_teknik_dokumantasyon.md dosyası hazırlanarak repoya eklenmiştir.  
...
###  Uygulanan Teknik İyileştirmeler
...
Projenin 6. haftasında, Enes Zukra tarafından geliştirilen ve optimize edilen CNN modelinin farklı klinik senaryolarda performansı test edilmiş, Accuracy, Precision, Recall ve F1 metrikleri analiz edilerek sonuçlar raporlanmıştır.

#### Kullanılan Bileşenler

- Test edilen model: Enes Zukra — CNN (TensorFlow/Keras), Hafta 6 optimizasyonu sonrası %92.4 doğruluk
- Ön işleme modülü: Selim Yağbasan — `data_preprocessing.py` (normalizasyon, aykırı değer baskılama, bellek optimizasyonu)

#### Test Senaryoları

- **Senaryo 1 — Temel Sınıflandırma:** Normal / Anormal ikili sınıflandırma testi gerçekleştirilmiştir.
- **Senaryo 2 — Melanom Tespiti:** Yanlış negatifi minimize etmek amacıyla karar eşiği 0.50'den 0.35'e düşürülmüştür.
- **Senaryo 3 — Diyabetik Retinopati:** Fundus görüntüleri üzerinde DR tespit testi yapılmıştır.
...
- Melanom senaryosunda Recall hedefi olan %90'ın üzerine çıkılmış (%92.31), eşik düşürme stratejisi başarıyla uygulanmıştır.
